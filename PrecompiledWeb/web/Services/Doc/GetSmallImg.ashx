<%@ WebHandler Language="C#" Class="GetSmallImg" %>
...
public class GetSmallImg : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ctx = context;
        string Path=Tool.Function.Request.GetRequestSrtring(ctx,"path");
        if (string.IsNullOrEmpty(Path))
        {
            return;
        }
        CreateImageOutput(50, 50, Path);
    }
    ...
    private void OutputImgToPage(System.Drawing.Bitmap bmp)
    {
        ...
        ctx.Response.ClearContent(); //需要输出图象信息 要修改HTTP头 
        byte[] buffer = ms.ToArray();

        ctx.Response.AddHeader("Content-type", "image/jpeg");
