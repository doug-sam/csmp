...
public partial class page_Inport_Class : _Sys_Inport
{
    ...
    protected void BtnCheck_Click(object sender, EventArgs e)
    {
        ...
        if (!dt.Columns.Contains("大类"))
        {
            Function.AlertBack("格式错误，系统找不到列<大类>"); return;
        }
        if (!dt.Columns.Contains("中类"))
        {
            Function.AlertBack("格式错误，系统找不到列<中类>"); return;
        }
        if (!dt.Columns.Contains("小类"))
        {
            Function.AlertBack("格式错误，系统找不到列<小类>"); return;
        }
        if (!dt.Columns.Contains("SLA"))
        {
            Function.AlertBack("格式错误，系统找不到列<SLA>"); return;
        }
        if (!dt.Columns.Contains("客户"))
        {
            Function.AlertBack("格式错误，系统找不到列<客户>"); return;
        }
        if (!dt.Columns.Contains("优先级"))
        {
            Function.AlertBack("格式错误，系统找不到列<优先级>"); return;
        }
        #endregion
