...
namespace CSMP.DAL
{
    public class SlaModeDAL
    {
        private const string ALL_PARM = "  ID,f_Name ";
        private const string FROM_TABLE = " from [sys_SlaMode] ";
        private const string TABLE = " sys_SlaMode ";
        private const string INSET = " (f_Name) values(@Name)  ";
        private const string UPDATE = " f_Name=@Name ";

        #region ReadyData
        private SlaModeInfo GetByDataReader(SqlDataReader rdr)
        {
            SlaModeInfo info = new SlaModeInfo();
            info.ID = Convert.ToInt32(rdr["ID"]);
            info.Name = rdr["f_Name"].ToString();
            return info;
        }

        private SqlParameter[] GetParameter(SlaModeInfo info)
        {
        ...
        /// 获取列表
        /// </summary>
        public List<SlaModeInfo> GetList(int PageSize, int CurPage, string StrWhere, out int Count)
        {
            List<SlaModeInfo> list = new List<SlaModeInfo>();
        ...
        public List<SlaModeInfo> GetList(string StrWhere)
        {
            List<SlaModeInfo> list = new List<SlaModeInfo>();
        ...
        public int Add(SlaModeInfo info)
        {
            StringBuilder strSQL = new StringBuilder();
        ...
        /// </summary>
        /// <param name="info">info</param>
        public bool Edit(SlaModeInfo info)
        {
            StringBuilder strSQL = new StringBuilder();
